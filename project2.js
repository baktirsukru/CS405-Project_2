...
class MeshDrawer {
	...
	constructor() {
		...
		this.lightPos = [1.0, 1.0, 1.0]; // default light position
		this.ambientIntensity = 0.1; // default ambient light intensity
		this.enableLighting = false; // lighting is initially off
...
const meshFS = `
			...
			void main()
			{
				...
				vec4 diffColor = diff * texture2D(tex, v_texCoord);
				vec4 ambientColor = ambient * texture2D(tex, v_texCoord);
				vec4 color = diffColor + ambientColor;
